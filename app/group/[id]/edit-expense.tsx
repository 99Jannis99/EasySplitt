...
export default function EditExpenseScreen() {
  ...
  const expense = useSelector((s: RootState) =>
    s.groups.expenses.find((e) => e.id === expenseId && e.groupId === id)
  );
  ...
  useEffect(() => {
    if (expense && group) {
      ...
      setSplitBetweenIds(
        expense.splitBetweenIds.length === group.participants.length
          ? []
  ...
  if (!expense) {
    return (
      <View style={styles.centered}>
        <Text>Ausgabe nicht gefunden.</Text>
      </View>
    );
  }
  ...
  const toggleSplit = (participantId: string) => {
    if (isSelected(participantId)) {
      const newSelected = selectedIds.filter((pid) => pid !== participantId);
